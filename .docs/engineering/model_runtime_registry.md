...
主要生成 LLM 现在已经接入 SiliconFlow API，默认模型为 `Qwen/Qwen3.6-27B`；DeepSeek API 仍保留同协议 provider 接口，用于对照和备份。

## 当前已部署或已接入

| 层级 | 名称 | 类型 | 当前状态 | 作用 | 备注 |
| --- | --- | --- | --- | --- | --- |
| Document Parser | MinerU local `3.1.15` | PDF parsing / OCR / layout engine | 已本地部署并完成 27 篇 PDF 处理 | PDF -> MinerU artifact | 通过 `127.0.0.1:8000` 调用；未来公网服务端自托管，不使用天翼云 |
| Vector Store | Qdrant `1.18.0` | Vector database | 已本地部署并完成批量入库检索 | 存储 `rag_chunk`、`table_record`、`evidence_record` | 本地 collection `enzyme_immobilization_b10` 当前约 2508 points，后续需要改名 |
| Embedding | `BAAI/bge-base-en-v1.5` | Sentence embedding | 已接入 | 语义召回和检索 smoke | 768 维，`local_files_only: true`，只用于当前阶段检索 |
...
| --- | --- | --- | --- | --- | --- |
| Generator LLM | SiliconFlow API | Chat / generation LLM | P0 | 生成推荐固化剂、优化配方、解释证据和不确定性 | 默认生成服务商；已接入 streaming；不把 API key 写入仓库 |
